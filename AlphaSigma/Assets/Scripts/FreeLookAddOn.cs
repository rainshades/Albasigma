...
namespace Albasigma
{
    ...
    public class FreeLookAddOn : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (UsingKeypad)
            {
                _freeLookComponent.m_XAxis.Value += RotationVector.x * LookSpeed * Time.deltaTime;
                _freeLookComponent.m_YAxis.Value += RotationVector.y * LookSpeed * Time.deltaTime;
            }
        }
        ...
        public void OnLook(InputAction.CallbackContext context)
        {
            ...
            if (UsingKeypad) 
            {
                RotationVector.x = lookMovement.x;
                RotationVector.y = lookMovement.y;
            }
            else
            {
